...
|----------------|------|------|--------|
| **Logo** | [`linkedin-logo.png`](../site/brand/linkedin-logo.png) | 400×400 | Square mark only (`>_`). If the logo tile shows the word **VAULTRUN**, you uploaded `lockup.png` — replace it. |
| **Cover / header** | [`linkedin-cover.jpg`](../site/brand/linkedin-cover.jpg) | 4200×700 | **Upload this JPEG** (LinkedIn’s help prefers JPEG over logo-PNGs). Company covers **are** a thin strip (~1128×191 on desktop) — that is the platform, not a failed crop. Two lines only. In the cropper: don’t zoom. Check **View as member**, not the tiny left preview (it clips the top). |
