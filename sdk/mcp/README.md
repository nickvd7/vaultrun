...
- **Extensions:** Tasks (`async=true` → `tasks/get` / `tasks/update` / `tasks/cancel`; caps via `MCP_TASK_TTL_SECONDS` / `MCP_TASK_MAX_INFLIGHT` / `MCP_TASK_MAX_AGE_SECONDS`), MCP Apps (`ui://vaultrun/session-panel`)
